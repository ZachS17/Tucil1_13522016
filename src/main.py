...
# cara input
print("Selamat datang di program Brute Force Cyberpunk 2077 Breach Protocol")
print("Bagiamana anda mau masukkan informasi?")
print("1. Random generate (Command Line Interface)")
print("2. File txt")
carainput = int(input("Masukkan pilihan anda: "))

if carainput == 1: # input data lengkap
    jtoken = int(input("Jumlah token unik: "))
    stringtoken = input("Token-token yang mungkin dengan spasi: ")
    arrtoken = [stringtoken[i:i+2] for i in range(0, len(stringtoken), 2)] # terima stringtoken jadi array
    buffer = int(input("Ukuran buffer (jumlah maksimal token yang disusun): "))
    ukuranmatriks = input("Ukuran matriks dengan format (baris kolom): ")
    baris, kolom = map(int, ukuranmatriks.split()) # pisah input jadi baris dan kolom
    jsekuens = int(input("Jumlah sekuens: "))
    ukuransekuens = int(input("Ukuran maksimal sekuens: "))

    # kosong untuk dirandom
    arrsekuens = []
    hadiahsekuens = []
    ...
    for i in range (jsekuens):
        randomlength = random.randint(2,buffer) # random panjangnya
        randomarray = random.sample(arrtoken,randomlength) # random sekuensnya
        randomvalue = random.randint(0,100) # random bobotnya
        arrsekuens.append(randomarray)
        hadiahsekuens.append(randomvalue)

    matriks = []
    for j in range (baris):
        satubaris = random.sample(arrtoken,kolom)
        matriks.append(satubaris)
                    ...
                    if indeks == buffer: # panjang sudah maksimal
                        ...
                        while True: # selama g ada halangan
                            if batasatas >= baris:
                                ...
                                if matrikskunjungan[sementara[indeks-1]][batasatas] != 1 and batasatas < kolom:
                                    batasatas += 1
                                    matriksbelokan[indeks].append(batasatas)
                                if matrikskunjungan[sementara[indeks-1]][batasbawah] != 1 and batasbawah >= 0:
                                    batasbawah -= 1
                                    matriksbelokan[indeks].append(batasbawah)
